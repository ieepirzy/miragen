...
**External sessions** (Claude Code / Codex / Grok Build launched from a shell or by mirarun, not by MiraGen) join the same substrate through `miragend`'s session plane and the stdlib-only `miragen-hook` adapter; the local `miragend` writes the Codex and Grok Build hooks, Codex hook trust and the bridge MCP entry itself (`harness_setup`) — see [docs/external-sessions.md](docs/external-sessions.md).

Model-tier agents get `memory_checkpoint` / `memory_remember` / `memory_read` tools; executor-tier agents reach the same surface through the `/mcp/memory` mount (point an `executor.mcp_servers` entry at it). **Native harness hooks**: the claude-code executor captures prompts, turn ends, tool failures, compactions and subagent completions through in-process SDK callbacks (when `claude-agent-sdk` is installed); the codex executor gets miragen-owned entries installed into `CODEX_HOME/hooks.json` (user entries preserved) pointing at the `miragen memory-hook codex` bridge, which reads one hook event on stdin, captures it idempotently, answers session starts with the guidance + working-state packet as `additionalContext`, and fails open — a broken memory service never blocks the agent. `hooks.mode: native_required` refuses to boot on executor kinds without a verified hook contract (kimi/grok: unverified, not unsupported). A write counts as saved only when the tool answers `accepted` — outages degrade **explicitly** (`persistence_unavailable`, a degraded line in the injected guide, counters on `/health`), never as "no relevant memories". Deliver both env vars into managed containers via `MIRAGEND_AGENT_ENV_PASSTHROUGH`.

The wire contract is the miragen-owned **[memory backend protocol](docs/memory-backend-protocol.md)** — Loimi is the blessed implementation, `backend: ephemeral` is the built-in in-process one (full lifecycle, zero durability, zero external services — dev/demo), and anything that passes `miragen memory-conformance <url>` (12 behavioral checks; `--ephemeral` self-tests the built-in) is a conforming backend.

**Recall** (`memory.recall`, on by default when a model is resolvable): at each boundary the optional lane runs a bounded hybrid search over the profile's read scopes, a **zero-or-more relevance selection** (one model call on cache misses; selecting nothing is normal), then canonically re-renders the chosen records under a budget — selector failure injects *nothing* rather than stuffing neighbors, and the packet says so. The extraction worker (`miragen memory-worker`, requires `memory.extraction.enabled`) consolidates captured events into proposed memories behind three demote-only gates, and with `--embed-url` also backfills dense-channel embeddings, with the space identity pinned by whatever endpoint actually embedded the text.
...
- **`grok-build`** — Grok Build via MIT `grok-build-client` (`headless` or `acp` transport; image installs CLI + client). See [docs/design/kimi-and-grok-executors.md](docs/design/kimi-and-grok-executors.md) and [packages/grok-build-client](packages/grok-build-client). A locked-down headless mode (`grok_tools`, `grok_permission_mode`, `grok_allow`/`grok_deny`, `grok_hermetic`, `grok_auth: subscription`) runs Grok with only the profile's MCP servers and no shell/file/web/subagent tools — see §6 Phase C of that doc.
...
`reviewed-publication/v1` means the **endpoint is supported** in this build;
a successful publish still requires the backend configured (otherwise 400).
`GET /health` exposes `publication.backend_configured` for readiness. Not
auto-called on executor success. Full reference:
[docs/design/reviewed-publication.md](docs/design/reviewed-publication.md).

### Example profiles

A Claude Code worker with a publication backend (reviewed graduation only):
...
executor:
  ...
  artifact_sink:               # backend for POST /runs/{id}/publications only
    kind: loimi
    url: https://loimi.mesh/mcp/
    bearer_token_env: LOIMI_TOKEN
```
