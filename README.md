...
**External sessions** (Claude Code / Codex launched from a shell, not by MiraGen) join the same substrate through `miragend`'s session plane and the stdlib-only `miragen-hook` adapter — see [docs/external-sessions.md](docs/external-sessions.md).
